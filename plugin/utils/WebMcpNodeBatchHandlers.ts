...
const createAtomicCommand = ( editor: MrppEditor, commands: any[] ) => {
	...
	command.execute = function () {
		...
		const signal = editor.signals.sceneGraphChanged;
		const previousActive = signal.active;
		signal.active = false;
		const executed: any[] = [];
		try {
			...
			for ( let index = executed.length - 1; index >= 0; index -- ) {
				...
				try {
					...
					// Continue rolling back the remaining already-executed commands.
			...
			signal.active = previousActive === undefined ? true : previousActive;
			signal.dispatch();
