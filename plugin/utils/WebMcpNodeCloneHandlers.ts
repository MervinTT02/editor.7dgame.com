...
const copyComponentsWithNewUUIDs = ( source: any, target: any ) => {

	for ( const key of [ 'components', 'commands' ] ) {

		if ( ! source[ key ] ) continue;
		target[ key ] = JSON.parse( JSON.stringify( source[ key ] ) );
		target[ key ].forEach( ( item: any ) => {

			if ( item.parameters?.uuid ) item.parameters.uuid = THREE.MathUtils.generateUUID();
			if ( item.parameters?.options ) {

				for ( const oldUuid of Object.keys( item.parameters.options ) ) {

					const newUuid = THREE.MathUtils.generateUUID();
					item.parameters.options[ newUuid ] = item.parameters.options[ oldUuid ];
					delete item.parameters.options[ oldUuid ];
...
const copyHierarchyDataWithNewUUIDs = ( source: any, target: any ) => {
	...
	const childCount = Math.min( source.children?.length ?? 0, target.children?.length ?? 0 );
	for ( let index = 0; index < childCount; index ++ ) {

		copyHierarchyDataWithNewUUIDs( source.children[ index ], target.children[ index ] );
